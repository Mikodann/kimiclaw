...
export default function IsometricThemeParkMobile() {
  ...
  const parkAttractiveness = useMemo(() => {
    ...
    const amenity = builtFacilities.filter((f) => f.id === "restroom").reduce((a, f) => a + (f.satisfactionBonus || 0), 0);
    return rideFun + amenity + Math.min(visitors.length, 100) * 0.25;
  ...
  const popularity = useMemo(() => {
    return [...builtFacilities]
      .sort((a, b) => (b.meta.users || 0) - (a.meta.users || 0))
      .slice(0, 5)
      .map((f) => ({ name: `${f.emoji} ${f.name}`, users: f.meta.users || 0 }));
  }, [builtFacilities]);

  const filteredBuildOptions = useMemo(() => {
    const all = Object.values(BUILDINGS).filter((b) => b.category === activeCategory);
    if (activeCategory !== "ride") return all;
    return all.filter((b) => !b.lockedByResearch || unlockedRides.includes(b.id));
  }, [activeCategory, unlockedRides]);

  useEffect(() => {
    const value = clamp(Math.round(0.55 * avgSatisfaction + 0.45 * Math.min(100, parkAttractiveness)), 0, 100);
    setRating(value);
  ...
  }, [running, speed, tiles, facilityMeta, visitors, parkAttractiveness, activeEvent]);
  ...
  const tickOneMinute = () => {
    ...
    if (activeEvent.minutesLeft > 0) {
      ...
      if (activeEvent.minutesLeft === 1) {
        setEventBanner("이벤트 종료");
      }
    ...
    const spawnChance = clamp((0.06 + parkAttractiveness / 280) * activeEvent.visitorMult, 0.04, 0.45);
      ...
      next = next
        .map((v) => {
          ...
          const candidates = dirs
            ...
            .filter((p) => p.x >= 0 && p.x < MAP_SIZE && p.y >= 0 && p.y < MAP_SIZE && pathSet.has(keyOf(p.x, p.y)));
          ...
          let satDelta = -0.4 + activeEvent.satDelta;
          ...
          return { ...v, x: nx, y: ny, satisfaction: newSat };
        })
    ...
    const income = passiveShopIncome + activityIncome;
  ...
  const placeBuilding = (x, y) => {
    ...
    if (selectedBuild !== "path") {
      const k = keyOf(x, y);
      setFacilityMeta((prev) => ({ ...prev, [k]: { level: 1, users: 0, revenue: 0, broken: false } }));
    }
  ...
  const tryOpenFacilityPopup = (x, y) => {
    ...
    if (tile?.type === "facility" && tile.facilityId) {
      setSelectedFacilityKey(keyOf(x, y));
      return true;
    }
    return false;
  };
  ...
  const startResearch = (rideId) => {
    ...
    if (researching || unlockedRides.includes(rideId)) return;
    const cost = costs[rideId] || 1000;
    if (money < cost) return;
    setMoney((m) => m - cost);
    setResearching(rideId);
    setResearchLeft(times[rideId] || 3);
  ...
  const handlePointerDown = (e) => {
    ...
    if (e.touches && e.touches.length === 2) {
      ...
      gestureRef.current = { mode: "pinch", startDist: dist, startZoom: cam.zoom, startCamX: cam.x, startCamY: cam.y };
      return;
    }
  ...
  const handlePointerMove = (e) => {
    ...
    if (gestureRef.current.mode === "drag") {
      ...
      setCam((prev) => ({ ...prev, x: gestureRef.current.startCamX + dx, y: gestureRef.current.startCamY + dy }));
    }
    ...
    <div className="h-screen w-full bg-gradient-to-b from-amber-50 to-lime-100 text-slate-800 flex flex-col overflow-hidden select-none">
      <header className="px-3 pt-3 pb-2 z-20">
        <div className="rounded-2xl bg-white/90 backdrop-blur shadow-md p-2.5">
          <div className="grid grid-cols-3 gap-2 text-center text-sm">
            ...
            <div className="rounded-xl bg-amber-50 p-2 min-h-[44px]"><div className="text-[11px] text-amber-700">공원 평점</div><div className="font-bold text-amber-800">{rating} / 100</div></div>
          </div>
      ...
      {panel === "park" ? (
          ...
          >
            <div className="absolute left-0 top-0" style={{ transform: `translate(${cam.x}px, ${cam.y}px) scale(${cam.zoom})`, transformOrigin: "0 0", willChange: "transform" }}>
              {Array.from({ length: MAP_SIZE }).map((_, x) => Array.from({ length: MAP_SIZE }).map((__, y) => {
                ...
                return <div key={`${x}-${y}`} className={`absolute ${bg} border border-white/45`} style={{ width: TILE_W, height: TILE_H, left: pos.x - TILE_W / 2, top: pos.y - TILE_H / 2, transform: "skewY(-26.565deg) scaleY(0.5)", borderRadius: 2, boxSizing: "border-box", boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.2)" }} />;
              }))}

              {builtFacilities.map((f) => {
                const pos = isoToScreen(f.x, f.y);
                return (
                  <div key={`f-${f.key}`} className="absolute text-2xl drop-shadow-md" style={{ left: pos.x - 12, top: pos.y - 32, transform: "translateZ(0)" }}>
                    {f.emoji}{f.meta.broken ? "❌" : ""}
                  </div>
                );
              })}
              ...
              {visitors.map((v) => {
                ...
                return <div key={v.id} className="absolute transition-all duration-500 ease-linear" style={{ left: pos.x - 6, top: pos.y - 12 }}><div className="w-3 h-3 rounded-full bg-sky-600 border border-white shadow" /></div>;
              })}

              {hoverTile && isInside(hoverTile.x, hoverTile.y) && (() => {
                const pos = isoToScreen(hoverTile.x, hoverTile.y);
                return <div className={`absolute border-2 ${placeValid ? "border-emerald-500/80" : "border-red-500/90"}`} style={{ width: TILE_W, height: TILE_H, left: pos.x - TILE_W / 2, top: pos.y - TILE_H / 2, transform: "skewY(-26.565deg) scaleY(0.5)", borderRadius: 2, pointerEvents: "none" }} />;
              })()}
            </div>
        ...
        <main className="flex-1 px-3 pb-2 overflow-y-auto">
          <div className="rounded-2xl bg-white p-3 shadow mb-3">
            <div className="font-semibold mb-2">최근 10분 수입/지출</div>
            <div className="flex items-end gap-1 h-24">
              {Array.from({ length: Math.max(incomeHistory.length, expenseHistory.length) }).map((_, i) => {
                const inc = incomeHistory[i] || 0;
                const exp = expenseHistory[i] || 0;
                return (
                  <div key={i} className="flex-1 flex items-end gap-[2px]">
                    <div className="w-1/2 bg-emerald-400 rounded-t" style={{ height: `${(inc / barMax) * 100}%` }} />
                    <div className="w-1/2 bg-rose-400 rounded-t" style={{ height: `${(exp / barMax) * 100}%` }} />
                  </div>
                );
              })}
            </div>
            <div className="text-[11px] text-slate-500 mt-1">초록=수입, 빨강=지출</div>
          </div>

          <div className="rounded-2xl bg-white p-3 shadow mb-3">
            <div className="font-semibold mb-2">인기 시설 순위</div>
            {popularity.length ? popularity.map((p, idx) => <div key={p.name} className="flex justify-between text-sm py-1"><span>{idx + 1}. {p.name}</span><span>{p.users}회</span></div>) : <div className="text-sm text-slate-500">아직 데이터가 없어요.</div>}
          </div>

          <div className="rounded-2xl bg-white p-3 shadow">
            <div className="font-semibold mb-2">방문객 만족도 추이</div>
            <div className="flex items-end gap-1 h-20">
              {satHistory.map((s, i) => <div key={i} className="flex-1 bg-blue-400 rounded-t" style={{ height: `${clamp(s, 5, 100)}%` }} />)}
            </div>
          </div>
        </main>
      )}
      ...
      <footer className="px-2 pb-2 z-20">
        <div className="rounded-2xl bg-white/95 shadow-lg p-2 mb-2">
          ...
          <div className="flex gap-2 overflow-x-auto">
            {["ferris", "bumper", "waterslide"].map((id) => {
              const b = BUILDINGS[id];
              const unlocked = unlockedRides.includes(id);
              return (
                <button key={id} onClick={() => startResearch(id)} className="min-w-[120px] min-h-[44px] rounded-xl px-2 py-1 text-left border border-slate-200 bg-slate-50">
                  <div className="text-sm">{b.emoji} {b.name}</div>
                  <div className="text-[11px] text-slate-500">{unlocked ? "해금 완료" : researching === id ? `연구중 ${researchLeft}분` : "탭해서 연구"}</div>
                </button>
              );
            })}
          </div>
        ...
        <div className="rounded-2xl bg-white/95 shadow-lg p-2">
          <div className="grid grid-cols-4 gap-2 mb-2">
            {CATEGORIES.map((cat) => (
              <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`min-h-[44px] rounded-xl text-xs font-semibold flex items-center justify-center gap-1 ${activeCategory === cat.id ? "bg-slate-800 text-white" : "bg-slate-100 text-slate-700"}`}>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {filteredBuildOptions.map((b) => (
              <button key={b.id} onClick={() => setSelectedBuild(b.id)} className={`min-w-[124px] min-h-[64px] rounded-xl px-2 py-1 text-left border ${selectedBuild === b.id ? "border-violet-500 bg-violet-50" : "border-slate-200 bg-white"}`}>
                <div className="flex items-center gap-1.5"><span className="text-xl">{b.emoji}</span><span className="font-semibold text-sm">{b.name}</span></div>
                <div className="text-[11px] text-slate-600">건설 ₩{b.cost.toLocaleString()}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      {selectedFacility && (
        <div className="absolute inset-0 bg-black/35 flex items-center justify-center z-30 p-4">
          <div className="w-full max-w-sm bg-white rounded-2xl p-4 shadow-xl">
            <div className="flex justify-between items-center mb-2">
              <div className="font-bold">{selectedFacility.building.emoji} {selectedFacility.building.name}</div>
              <button className="min-w-[44px] min-h-[44px]" onClick={() => setSelectedFacilityKey(null)}>✕</button>
            </div>
            <div className="text-sm space-y-1 mb-3">
              <div>상태: {selectedFacility.meta.broken ? "고장" : "정상"}</div>
              <div>누적 이용객: {selectedFacility.meta.users || 0}명</div>
              <div>누적 수익: ₩{(selectedFacility.meta.revenue || 0).toLocaleString()}</div>
              <div>레벨: Lv.{selectedFacility.meta.level || 1}</div>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <button className="min-h-[44px] rounded-xl bg-indigo-100" onClick={upgradeFacility}>업그레이드</button>
              <button className="min-h-[44px] rounded-xl bg-amber-100" onClick={repairFacility}>수리</button>
              <button className="min-h-[44px] rounded-xl bg-rose-100" onClick={demolishFacility}>철거</button>
            </div>
          </div>
        </div>
      )}
